...
TEMPLATE_XLSX = os.path.join(BASE_DIR, "Fom_Prom.xlsx") 

OUTPUT_DIR  = r"C:\Users\maxim\OneDrive\Документы\Shops"
OUTPUT_NAME = "buktex_prom_ready.xlsx"
...
def make_driver():
    ...
    # Современный скрытый режим (сайты видят его как обычный видимый браузер)
    ...
    # Маскировка от систем анти-бот
    opts.add_argument("--disable-blink-features=AutomationControlled")
    opts.add_experimental_option("excludeSwitches", ["enable-automation", "enable-logging"])
    opts.add_experimental_option("useAutomationExtension", False)
    
    # Обязательно задаем большое окно, иначе мобильная верстка всё сломает
    opts.add_argument("--window-size=1920,1080")
    opts.add_argument("--log-level=3")
    
    # Прикидываемся обычным пользователем Windows
    opts.add_argument("user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36")

    service = Service(ChromeDriverManager().install())
    driver = webdriver.Chrome(service=service, options=opts)
    
    # Скрываем свойство webdriver через JavaScript (самая важная часть защиты)
...
def get_product_links(driver, cat_url):
    ...
    # Случайная задержка от 2 до 4 секунд, чтобы имитировать чтение страницы человеком
...
def parse_product_page(driver, url):
    ...
    # Случайная задержка при переходе на товар (1.5 - 2.5 сек)
    time.sleep(random.uniform(1.5, 2.5))
    soup = BeautifulSoup(driver.page_source, "lxml")
...
def main():
    if not os.path.exists(TEMPLATE_XLSX):
        print(f"[!] Файл шаблона {TEMPLATE_XLSX} не найден!")
        return

    prod_headers = pd.read_excel(TEMPLATE_XLSX, sheet_name="Export Products Sheet").columns.tolist()
    group_headers = pd.read_excel(TEMPLATE_XLSX, sheet_name="Export Groups Sheet").columns.tolist()
    ...
    try:
        ...
        for cat_url, info in all_categories.items():
            ...
            for link in links:
                try:
                    ...
                    row = {h: "" for h in prod_headers}
                    row.update({
                        "Код_товару": p.sku,
                        "Назва_позиції": p.name,
                        "Назва_позиції_укр": p.name,
                        "Опис": p.description,
                        "Опис_укр": p.description,
                        "Ціна": p.price_uah,
                        "Валюта": "UAH",
                        "Тип_товару": "r",
                        "Одиниця_виміру": "шт.",
                        "Посилання_зображення": ", ".join(p.images),
                        "Наявність": p.availability_pm,
                        "Номер_групи": gid,
                        "Назва_групи": gname,
                        "Ідентифікатор_товару": f"{gid}-{p.sku if p.sku else hashlib.md5(link.encode()).hexdigest()[:8]}",
                        "Продукт_на_сайті": p.url
                    })
        ...
        groups_rows = []
        for gid, gname in used_groups.items():
            grow = {h: "" for h in group_headers}
            grow.update({"Номер_групи": gid, "Назва_групи": gname})
            groups_rows.append(grow)

        if not os.path.exists(OUTPUT_DIR): os.makedirs(OUTPUT_DIR)
        save_path = os.path.join(OUTPUT_DIR, OUTPUT_NAME)
        
        with pd.ExcelWriter(save_path, engine="openpyxl") as writer:
            pd.DataFrame(all_products, columns=prod_headers).to_excel(writer, index=False, sheet_name="Export Products Sheet")
            pd.DataFrame(groups_rows, columns=group_headers).to_excel(writer, index=False, sheet_name="Export Groups Sheet")

        print(f"\n[ГОТОВО] Файл для Prom.ua успешно сохранен: {save_path}")
